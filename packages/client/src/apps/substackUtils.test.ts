 ...
 *
 * Invariants tested:
 * - INV-1: Markdown syntax renders as formatted HTML (renderMarkdown)
 * - INV-2: Read time estimation is at least 1 and scales with word count
 */
...
describe("estimateReadTime", () => {
  it("INV-2: returns minimum 1 for empty string", () => {
    expect(estimateReadTime("")).toBe(1);
  });

  it("INV-2: returns minimum 1 for very short text", () => {
    expect(estimateReadTime("Hello world")).toBe(1);
  });

  it("INV-2: scales with word count (200 words ≈ 1 minute)", () => {
    const words = Array(200).fill("word").join(" ");
    expect(estimateReadTime(words)).toBe(1);
  });

  it("INV-2: 400 words ≈ 2 minutes", () => {
    const words = Array(400).fill("word").join(" ");
    expect(estimateReadTime(words)).toBe(2);
  });

  it("INV-2: longer articles get higher read time", () => {
