...
export const MemoApp = React.memo(function MemoApp(_: AppProps) {
  return (
      ...
      <div className="w-44 bg-neutral-50 border-r border-neutral-200 flex flex-col shrink-0">
        ...
        <div className="overflow-y-auto flex-1 p-2 space-y-0.5">
          {PAGES.map((page, i) => (
            <div
              key={page}
              className={`px-2 py-1.5 rounded cursor-pointer text-xs ${i === 0 ? "bg-blue-100 text-blue-800 font-medium" : "text-neutral-600 hover:bg-neutral-100"}`}
            >
              📄 {page}
            </div>
          ))}
              ...
              if (line.startsWith("| ") && !line.startsWith("| -")) {
                ...
                return (
                  <tr key={i} className="border-b border-neutral-200">
                    {cells.map((c, j) => (
                      <td key={j} className={`px-2 py-1 text-xs ${i === 17 ? "font-semibold bg-neutral-50" : ""}`}>{c}</td>
                    ))}
                  </tr>
                );
              }
